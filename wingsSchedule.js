...
function formatDateCellData(dateStringAndData) {
  ...
  const dateString = dateStringAndData.dateString.toString();


  const game = gameItemInfo?.dates[0]?.games[0];

    formatted = `<div class="date_item | grid gap-24 padding-24">
      <div class="date_item__full-date | fs-200 margin-block-end-12">${moment(dateString, "YYYY-MM-Do").format("dddd, MMMM Do YYYY")}</div>
